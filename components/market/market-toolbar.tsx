...
export function MarketToolbar({ locale, initialQuery, initialSort }: {
  locale: Locale;
  initialQuery: string;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Input
        ...
        className="h-9 max-w-[300px] flex-[1_1_220px] rounded-full bg-secondary px-4 text-xs"
      />
      <Select
        value={sort}
        onValueChange={v => { const s = v as MarketSort; setSort(s); push(query, s); }}
      >
        <SelectTrigger size="sm" className="h-9 rounded-full bg-secondary text-xs" aria-label={t('sortBy', locale)}>

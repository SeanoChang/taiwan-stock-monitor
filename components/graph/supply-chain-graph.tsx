...
export function SupplyChainGraph({ locale, focus }: { locale: Locale; focus?: string }) {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-background text-foreground">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-10 px-6 pt-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <p className="text-[11px] text-foreground/55">
            {TW_COUNT} {t('graphCounts', locale)} · {TOTAL_COUNT} {t('nodes', locale)} · {CATEGORIES.length} {t('segments', locale)}
          </p>
          <Input
            value={query}
            onChange={e => { setQuery(e.target.value); setSelection(null); setGroupFilter(null); }}
            placeholder={t('searchPlaceholder', locale)}
            className="ss-veil pointer-events-auto h-9 max-w-[300px] flex-[1_1_200px] rounded-full px-4 text-xs"
          />
          {matches && (
            <span className="text-[11.5px] text-primary">{matches.size} {t('matches', locale)}</span>
          )}
          <div className="pointer-events-auto ml-auto flex items-center gap-2">
            <LocaleToggle locale={locale} />
            <NavLinks locale={locale} current="/supply-chain" />
            <Badge variant="outline" className="rounded-full border-border px-2.5 py-1 text-[10px] font-normal text-foreground/45">
              {t('notAdvice', locale)}
            </Badge>
          </div>
        </div>
        ...
        <div className="pointer-events-auto mt-3 flex flex-wrap items-center gap-1.5">
          {GROUP_LABELS.map((label, i) => (
            <button
              key={i}
              onClick={() => { setGroupFilter(groupFilter === i ? null : i); setSelection(null); }}
              className={cn(
            ...
            >
              <span className="size-[9px] flex-none rounded-full" style={{ background: PALETTE[i] }} />
              {pick(label, locale)}
            </button>
          ))}
          {(groupFilter !== null || selection || query) && (
            <Button
              ...
              size="sm"
              onClick={clearAll}
              className="h-7 rounded-full border-primary/40 bg-secondary px-2.5 text-[11.5px] font-semibold text-primary"
            >
      ...
      <p className="pointer-events-none absolute bottom-5 left-6 z-10 max-w-[520px] text-[11.5px] leading-relaxed text-foreground/45">
        {t('graphHint', locale)}
      </p>

      {selection && (
        <NodePanel nodeId={selection} locale={locale} onNavigate={navigate} onClose={() => setSelection(null)} />
      )}
    </div>
  );
}

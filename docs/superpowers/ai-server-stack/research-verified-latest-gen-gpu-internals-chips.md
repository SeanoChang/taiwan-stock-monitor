...
- **Tier:** **[s] sourced** (each claim has a URL; not yet adversarially re-voted).
  Build spec: `plans/007-modernize-server-rubin-gpu-internals-and-chips.md`.

## 1. Generation — the model is one gen behind

The tree/scene model **GB300 NVL72 (Blackwell Ultra)** = the *currently-shipping*
...
- **Rubin CPX** (Sept 2025, avail. end-2026): context/prefill inference GPU, **128GB
  GDDR7, no HBM, no advanced packaging** — a *different, cheaper* supply chain. The
...
the gap is *nodes*, not companies. Priority additions (HIGH first):
...
**Caveat:** exact per-board chip *counts* sit behind SemiAnalysis's paid BOM models;

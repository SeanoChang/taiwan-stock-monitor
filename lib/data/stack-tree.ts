...
const NODE_TIER: Record<string, Confidence> = {
  ...
  'rubin.ultra': 'gap',
  'rubin.feynman': 'gap',
  // sourced — board power silicon (Plan 007 Part C). Categories sourced from
  // teardowns; exact per-board counts sit behind paid BOMs → never quantified
  // as verified. e-fuse is foreign-only (no TW companyIds).
  'rack.tray.power': 'sourced',
  'rack.tray.power.drmos': 'sourced',
  'rack.tray.power.vrm': 'sourced',
  'rack.tray.power.pmic': 'sourced',
  'rack.tray.power.efuse': 'sourced',
  'rack.tray.retimer': 'sourced',
  'rack.tray.norflash': 'sourced',
  'rack.tray.pcieswitch': 'sourced',
  'rack.tray.rot': 'sourced',
  'rack.tray.cpld': 'sourced',
  'rack.tray.ssd': 'sourced',
  'rack.tray.timing': 'sourced',
  'rack.tray.monitor': 'sourced',
  'rack.tray.mgmtphy': 'sourced',
...
const CONTAINMENT_NODES: StackNode[] = [
  ...
  },
  {
    ...
    edges: [
      ...
      edgeTo('rack.tray.power', 'containment', SRC_LATEST),
      edgeTo('rack.tray.retimer', 'containment', SRC_LATEST),
      edgeTo('rack.tray.norflash', 'containment', SRC_LATEST),
      edgeTo('rack.tray.pcieswitch', 'containment', SRC_LATEST),
      edgeTo('rack.tray.rot', 'containment', SRC_LATEST),
      edgeTo('rack.tray.cpld', 'containment', SRC_LATEST),
      edgeTo('rack.tray.ssd', 'containment', SRC_LATEST),
      edgeTo('rack.tray.timing', 'containment', SRC_LATEST),
      edgeTo('rack.tray.monitor', 'containment', SRC_LATEST),
      edgeTo('rack.tray.mgmtphy', 'containment', SRC_LATEST),
    ],
  },
  {
    id: 'rack.tray.grace',
  ...
  {
    id: 'rack.tray.power',
    name: l('Board Power ICs', '板載電源 IC'),
    blurb: l(
      'DrMOS, multiphase VRM, point-of-load PMIC and e-fuse silicon on the GPU/CPU board — dozens per GPU, 1000A+ core rails.',
      'GPU／CPU 板上的 DrMOS、多相 VRM、負載點 PMIC 與 e-fuse 晶片 — 每 GPU 數十顆，核心軌 1000A+。',
    ),
    categoryId: 'analog',
    stageId: 'chip',
    edges: [
      edgeTo('rack.tray.power.drmos', 'containment', SRC_LATEST),
      edgeTo('rack.tray.power.vrm', 'containment', SRC_LATEST),
      edgeTo('rack.tray.power.pmic', 'containment', SRC_LATEST),
      edgeTo('rack.tray.power.efuse', 'containment', SRC_LATEST),
    ],
  },
  {
    id: 'rack.tray.power.drmos',
    name: l('DrMOS / Smart Power Stage', 'DrMOS／智慧功率級'),
    blurb: l(
      'Driver+MOSFET power stages, dozens per GPU board, feeding 1000A+ core rails.',
      '驅動＋MOSFET 功率級，每 GPU 板數十顆，供應 1000A+ 核心軌。',
    ),
    categoryId: 'analog',
    companyIds: ['upi6719', 'gmt', 'anpec'],
    stageId: 'chip',
    specs: [spec('driver+MOSFET; dozens/GPU board', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.power.vrm',
    name: l('Multiphase VRM / PWM Controller', '多相 VRM／PWM 控制器'),
    blurb: l(
      '12V → ~0.8V multiphase controllers for the GPU/CPU core rails.',
      '12V → 約 0.8V 多相控制器，供應 GPU／CPU 核心軌。',
    ),
    categoryId: 'analog',
    companyIds: ['upi6719', 'gmt', 'silergy'],
    stageId: 'chip',
    specs: [spec('12V → ~0.8V multiphase', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.power.pmic',
    name: l('Point-of-Load PMIC', '負載點 PMIC'),
    blurb: l(
      'Point-of-load PMICs for NIC/DPU/BMC/module rails (incl. the Rubin SOCAMM module).',
      '供應 NIC／DPU／BMC／模組軌（含 Rubin SOCAMM 模組）的負載點 PMIC。',
    ),
    categoryId: 'analog',
    companyIds: ['silergy', 'gmt', 'upi6719'],
    stageId: 'chip',
    specs: [spec('NIC/DPU/BMC/module rails', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.power.efuse',
    name: l('e-Fuse / Hot-Swap', 'e-Fuse／熱插拔'),
    blurb: l(
      'Busbar inrush limiting and per-tray OCP; foreign (Infineon, TI) — no Taiwan-listed play.',
      '匯流排突波限流與每托盤過流保護；外商（Infineon、TI）— 無台廠掛鉤。',
    ),
    categoryId: 'analog',
    stageId: 'chip',
    specs: [spec('inrush/OCP per tray (Infineon/TI)', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.retimer',
    name: l('PCIe/NVLink Retimer', 'PCIe／NVLink 中繼器'),
    blurb: l(
      'Re-clocks Gen6 signals over the 500mm+ midplane (critical on Rubin). Parade leads TW; Astera / Montage foreign.',
      '在 500mm+ 中板上重新定時 Gen6 訊號（Rubin 尤為關鍵）。譜瑞為台廠主力；Astera／Montage 為外商。',
    ),
    categoryId: 'icdesign',
    companyIds: ['parade'],
    stageId: 'chip',
    specs: [spec('re-clock Gen6 over 500mm+ midplane', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.norflash',
    name: l('Serial NOR Flash', '串列 NOR Flash'),
    blurb: l(
      'BMC/UEFI/VBIOS/FPGA config storage, many per board.',
      'BMC／UEFI／VBIOS／FPGA 設定儲存，每板多顆。',
    ),
    categoryId: 'memchip',
    companyIds: ['winbond', 'macronix'],
    stageId: 'chip',
    specs: [spec('BMC/UEFI/VBIOS/FPGA config', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.pcieswitch',
    name: l('PCIe Switch / Smart Fabric', 'PCIe 交換器／智慧網狀'),
    blurb: l(
      'Gen6 PCIe fan-out (CX-9 embeds one); foreign — Broadcom, Microchip, Astera.',
      'Gen6 PCIe 扇出（CX-9 內建一顆）；外商 — Broadcom、Microchip、Astera。',
    ),
    categoryId: 'icdesign',
    stageId: 'chip',
    specs: [spec('Gen6 PCIe fan-out (Broadcom/Microchip)', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.rot',
    name: l('TPM / Root-of-Trust', 'TPM／信任根'),
    blurb: l(
      'TPM 2.0 (confirmed on GB300) + platform root-of-trust.',
      'TPM 2.0（GB300 已確認）＋平台信任根。',
    ),
    categoryId: 'icdesign',
    companyIds: ['nuvoton'],
    stageId: 'chip',
    specs: [spec('TPM 2.0 + platform RoT', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.cpld',
    name: l('CPLD / FPGA Sequencing', 'CPLD／FPGA 定序'),
    blurb: l(
      'Power sequencing / board glue on every tray; foreign — Lattice, AMD.',
      '每托盤的電源定序／板級膠合邏輯；外商 — Lattice、AMD。',
    ),
    categoryId: 'icdesign',
    stageId: 'chip',
    specs: [spec('power sequencing / glue (Lattice)', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.ssd',
    name: l('NVMe / SSD Controller', 'NVMe／SSD 控制器'),
    blurb: l(
      'Boot + E1.S local storage controller. Phison (TW); Silicon Motion (SIMO) is US-ADR — no TW quote.',
      '開機與 E1.S 本地儲存控制器。群聯（台）；慧榮（SIMO）為美 ADR — 無台股報價。',
    ),
    categoryId: 'icdesign',
    companyIds: ['phison'],
    stageId: 'chip',
    specs: [spec('boot + E1.S NVMe controller', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.timing',
    name: l('Clock Gen + Crystal', '時脈產生＋石英'),
    blurb: l(
      'PCIe/Ethernet reference timing: TXC crystals (TW); Renesas/SkyWorks clock/jitter silicon (foreign).',
      'PCIe／乙太參考時脈：晶技石英（台）；Renesas／SkyWorks 時脈／抖動晶片（外商）。',
    ),
    categoryId: 'passives',
    companyIds: ['txc'],
    stageId: 'board',
    specs: [spec('PCIe/Eth reference timing', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.monitor',
    name: l('Temp/Volt/Power Monitor + EC', '溫度／電壓／功率監控＋EC'),
    blurb: l(
      'PMBus telemetry to the BMC (temp/volt/power) plus embedded controller.',
      '經 PMBus 回報 BMC 的遙測（溫度／電壓／功率）與嵌入式控制器。',
    ),
    categoryId: 'icdesign',
    companyIds: ['nuvoton'],
    stageId: 'chip',
    specs: [spec('PMBus telemetry + EC', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.tray.mgmtphy',
    name: l('Mgmt Ethernet PHY', '管理乙太 PHY'),
    blurb: l('1G RJ45 management-port PHY.', '1G RJ45 管理埠 PHY。'),
    categoryId: 'icdesign',
    companyIds: ['realtek'],
    stageId: 'chip',
    specs: [spec('1G mgmt Ethernet PHY', 'sourced', { sourceUrl: SRC_LATEST })],
    edges: [],
  },
  {
    id: 'rack.switchtray',
...
const SUBSYSTEM_MEMBERS: Record<string, string[]> = {
  'sub.compute': [
    ...
    'gpu.internal.gpc',
    'gpu.internal.tpc',
    'gpu.internal.sm.warp',
    'gpu.internal.sm.regfile',
    'gpu.internal.sm.l1',
    'gpu.internal.sm.tensor',
    'gpu.internal.sm.tmem',
    'gpu.internal.l2',
    'rubin.gpu',
    'rubin.gpu.sm',
    'rubin.vera',
  ],
  'sub.memory': ['gpu.hbm', 'mem.tiers', 'rubin.hbm', 'rack.tray.norflash', 'rack.tray.ssd'],
  'sub.packaging': [
    'gpu.cowos',
    'gpu.substrate',
    'gpu.substrate.film',
  ...
  'sub.scaleup': ['net.scaleup', 'rack.switchtray', 'rack.copper', 'rubin.nvswitch'],
  ...
  'sub.power': [
    ...
    // Plan 007:
    'rack.tray.power',
    'rack.tray.power.drmos',
    'rack.tray.power.vrm',
    'rack.tray.power.pmic',
    'rack.tray.power.efuse',
  ],
...
const STAGE_MEMBERS: Record<string, string[]> = {
  ...
  system: ['rack', 'rack.tray', 'mem.tiers', 'rack.rubin'],
  anchor: [
    'rack.tray.grace',
    'rack.switchtray',
    'gpu.internal.sm',
  ...
  board: ['gpu.substrate.drill', 'rack.tray.timing'],
